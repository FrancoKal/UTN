...
int main(void){
	...
	char ** cli;   			   //Listado de clientes conectados.
...
//Para que escuche solo en la dirección de config.txt
//	datosServer.sin_addr.s_addr = inet_addr(configuracion.ip);
//Para que escuche en cualquier dirección
		...
		for(i = 0; i <= fdmax; i++){
			if(FD_ISSET(i,&readset)){
				if(i == listener){     //Tengo conexión para aceptar.
					...
					printf("El cliente tuvo: %d, y fdmax estaba en: %d.\n", cliente, fdmax);
					if(cliente > fdmax){     //actualizo el máximo fichero.
						fdmax = cliente;
						cli = realloc(cli, sizeof(char *)*(fdmax+1));
						if (!cli) exit(3);
					}

					*(cli+cliente) = malloc(sizeof("255.255.255.255"));
					if (!(*(cli+cliente))) exit(3);
					strcpy(*(cli+cliente),inet_ntoa((struct in_addr)datosCliente.sin_addr));
					...
					recibir(i,recibido, MAX_DATOS,&readset);
					if (strcmp(recibido,"/salir")) {
						printf("%s envió: %s\n", *(cli+cliente),recibido);
					}
					else {
