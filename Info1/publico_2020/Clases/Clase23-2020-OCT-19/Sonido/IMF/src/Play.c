...
int main(int argc, char ** argv)
{
	...
	audio_params config = {{0, 0, 0}, RATE, SIZE, CHANNELS};
	audio_params sand = {{0, 0, 0}, -1, -1, -1};
	int buffsize;
	int salida = 0, i;
	int repro = 0;
	...
	if ((argc > 1) && (argc < 6))
	{
		for (i=1; i<argc; i++)
		{
			if((*(*(argv+i)) == '-') && (*(*(argv+i)+1) == 'r'))
				sand.v_muestreo = atoi((*(argv+i)+2));
			else if((*(*(argv+i)) == '-') && (*(*(argv+i)+1) == 's'))
				sand.t_muestra = atoi((*(argv+i)+2));
			else if((*(*(argv+i)) == '-') && (*(*(argv+i)+1) == 'c'))
				sand.c_canales = atoi((*(argv+i)+2));
	...
	if (!salida)
	{
		fread(&config, 1, sizeof(config), rawfd); /* lee */

		if ((config.f_id[0] != 'I') || (config.f_id[1] != 'M') || (config.f_id[2] != 'F')) 
		{
			fprintf(stderr,"Error en formato de archivo, formato no válido\n");
			salida = 6;
		}
	...
	if (!salida)
	{
		if (sand.v_muestreo != -1) config.v_muestreo = sand.v_muestreo;
		if (sand.t_muestra != -1) config.t_muestra = sand.t_muestra;
		if (sand.c_canales != -1) config.c_canales = sand.c_canales;
