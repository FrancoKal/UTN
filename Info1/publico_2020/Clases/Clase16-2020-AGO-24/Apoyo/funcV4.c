...
int carga (runner *** listado)
{
	...
	if (id_aux > 0)
	{
		...
		p_corr = (runner *) malloc (sizeof(runner));

	}

	while ((id_aux > 0) && (listaux) && (largo_nombre != 0) && (p_corr))
	{
		if (cant != 0)
		{
		...
		if ((listaux) && (p_corr))
		{
			...
			*(aux_nom+largo_nombre) = '\0';
			p_corr->nombre = (char *) malloc (largo_nombre*sizeof(char)+1);

			if (p_corr->nombre != NULL)
			{

				strcpy(p_corr->nombre, aux_nom);

				printf("Ingrese el tiempo utilizado (min): ");
				scanf("%d", &(p_corr->tiempo));
				printf("Ingrese la distancia recorrida (kmts): ");
				scanf("%f", &(p_corr->distancia));
...
void calculo (runner ** listado, velocidad *** resultado, int cant)
{
	...
	*resultado = (velocidad **) malloc (sizeof (velocidad *)*cant);
	...
	if (*resultado)
	{
		...
		{
			p_veloc = (velocidad *) malloc (sizeof(velocidad));
			if (p_veloc != NULL)
			{
				*((*resultado)+i) = p_veloc;

				largo_nombre = strlen (listado[i]->nombre);
				p_veloc->nombre = (char *) malloc (largo_nombre*sizeof(char)+1);

				if (p_veloc->nombre != NULL)
				{
					p_veloc->id = (*(listado+i))->id;
//					velocidad = distancia/(tiempo/60);
					p_veloc->veloc = (*(listado+i))->distancia/((*(listado+i))->tiempo/60.0);
					strcpy(p_veloc->nombre, listado[i]->nombre);
				}
				else
				{
